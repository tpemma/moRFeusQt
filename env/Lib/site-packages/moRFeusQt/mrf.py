...
class MoRFeus(object):
    ...
    def linear_sweep_bidirectional(self, start_freq, end_freq, step, delay):
        ...
        self.printProgressBar(0, stepcount, prefix='Linear Sweep:', suffix='', length=43)
            ...
            if stepcount == 0:
                ...
                if sock.IsConnected():
                    ...
                    for x in freq_steps:
                        ...
                        self.printProgressBar(y, stepcount, prefix='Sweep Progress:', suffix='', length=43)
                        t.sleep(delay / 1000)
                        y += 1
                    ...
                    self.message(self.SET, self.funcFrequency, start_freq / self.mil)
                    endtime = t.time()
                    sock.Close()
                    dwelltime = (delay, 'ms dwell', (endtime - starttime), 'sec', (step / 1000), 'MHz')
    ...
    def quadratic_sweep_bidirectional(self, start_freq, end_freq, step, delay):
        start_freq = int(start_freq * self.mil)
        end_freq = int(end_freq * self.mil)
        step = int(step * 1000)
        
        freq_steps_up = list(self.freqRange(start_freq, end_freq, step))
        freq_steps_down = list(self.freqRange(end_freq, start_freq, -step))
        freq_steps = freq_steps_up + freq_steps_down
        ...
        self.printProgressBar(0, stepcount, prefix='Quadratic Sweep:', suffix='', length=43)
        ...
        while True:
            if stepcount == 0:
                ...
                if sock.IsConnected():
                    starttime = t.time()
                    for x in freq_steps:
                        # Example of quadratic sweep, adjust as needed
                        freq_val = x if x <= end_freq else end_freq - (x - end_freq)**2 * step
                        self.message(self.SET, self.funcFrequency, (freq_val / self.mil))
                        sock.SetFreq("{0:8.6f}".format(freq_val))
                        self.printProgressBar(y, stepcount, prefix='Sweep Progress:', suffix='', length=43)
                        t.sleep(delay / 1000)
                        y += 1
                        power = sock.GetStrength()
                        freq.append(freq_val / self.mil)
                        powah.append(float(power[:-2]))
